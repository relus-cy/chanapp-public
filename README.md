...
自用缠论分析 web app（公开版）。v1.7.2 内置固定版 chan.py 计算结构和原生形态买卖点，支持严格/宽松成笔标准，前端 lightweight-charts
（本地 vendor，无构建）。

功能全景：5分/15分/30分/60分/日线 × A股指数/个股（内置演示数据的具体覆盖见「数据说明」）；自选股 + 交易时段自动刷新 + 后台预热；
...
规则开关收进信号摘要行右侧的「图层与规则」浮层；三周期摘要主体切换周期，信号尾钮打开右栏详情，依据卡和 AI 情景也可在右栏展开。侧栏支持固定、窄栏和悬停展开，切换浏览器标签页时收回悬停状态。底栏仅保留数据摘要，缓存状态收进名称旁标记。
